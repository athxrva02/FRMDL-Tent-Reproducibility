...
config — no code changes. `tent.py`, `norm.py`, and the method logic are untouched. Total GPU
work is ≈3 reproduction runs + ≈11 short sweep runs.
...
## Task split (2 students, distinct criteria)
...
> choice of the method), not as a separate "hyperparameter check".
...
- **Parallelize across Colab accounts.** B's BS and STEPS sweep points are independent, so they
  can be split across multiple Colab accounts to stay under the per-session time limit.
...
**STEPS=16** (16× backward per batch); those two points alone are ~half of B's GPU time.
...
- **Critical path: A → B.** A must finish first (its source/norm/tent runs feed B's ablation 1,
  and everyone needs the shared checkpoint/data).
...
5. Do our results uphold the paper's main conclusions? (required by grading rubric).
6. Per-student contribution paragraphs (required by assignment).
